...
当前项目已经具备原生 Android + FastAPI + RAG + Doubao / Mock 流式回复 + 商品卡片 + 详情页 + 反馈按钮的完整闭环。最近又补了 SKU 同系列规格、Markdown / 表格展示、商品对比、trace、groundedness、多模态图片输入、ASR 语音输入、TTS 播报、购买对象切换和无障碍设置相关能力。
...
5. **Demo 口径保守**：不承诺购物车、下单、库存、真实交易、图像向量搜同款、实时流式 ASR 或完整全品类深度标注。
...
| A9 | 上传安热沙/安耐晒防晒图片并发送补充需求 | 图片缩略图、image_plan、alias、商品卡片 | 图片理解线索进入 RAG；不说成图搜图 |
| A10 | 麦克风录入一条防晒 query | 录音、转写、用户确认发送 | 转写文本进入输入框；不自动误发 |
| A11 | 点击助手头像/播放按钮，切换详细播报 | TTS 播报、停止、详细/不详细模式 | 可播放、可停止；两种播报长短有差异 |
| A12 | 底部购买对象从 `自己` 切到 `妈妈` / `爸爸` | recipient context、隐私字段 | 本轮 request 带对象上下文；不展示真实地址电话 |
| A13 | 左侧抽屉新建会话后问泛问题 | 会话隔离、主动追问 | 新会话不继承旧防晒上下文；信息不足先追问 |
...
最终录屏按 `docs/submission_video_script.md` 的 10 分钟以内版本组织素材；如果要现场 60-90 秒快讲，再用下面的迷你版。
...
1. `/health` 确认真实模型和本次演示模型。
2. 安热沙/安耐晒图片输入与 text-first 多模态。
3. 底部购买对象切换和左侧新建对话。
4. 一次语音输入、一次小助手 TTS 播报、详细/不详细模式和字号/状态提示。
5. 优惠/库存/下单 guardrail 与 trace / claim audit 证据。
...
2. 图像向量搜同款、多图复杂理解和视频长序列理解。
3. 实时流式 ASR、长音频转写和云端语音账号体系。
4. 全量非美妆商品的深度标注。
