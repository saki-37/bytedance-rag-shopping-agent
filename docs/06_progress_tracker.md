...
2026-05-26 已补上 V1 检索层：`QueryIntent`、预算/排除条件硬约束、信息不足主动追问、`RetrievalTrace` 可解释输出，以及本地 debug 接口。当前仍不是最终参赛版本，最主要的缺口是：真实 Doubao 调用后的推荐编排还需要评测约束，Chroma 向量库还需要稳定验证，Graph-aware / hybrid retrieval 还没有进入主链路。
...
| RAG 基本链路 | 检索商品并基于资料回答 | 已完成 V1 | 结构化硬过滤 + 关键词/facet/可选 Chroma 召回 + 可解释 trace；还不是最终 hybrid RAG 管线 |
...
1. 构建 Chroma 索引，并确认后端运行时真实使用向量召回。
2. 让 `scripts/run_golden_queries.py` 输出 `RetrievalTrace`，形成最小 benchmark 表。
3. 接入真实 Doubao / ARK OpenAI-compatible API 后，验证 prompt 是否严格使用召回商品。
4. 扩展美妆增强数据：
   - 当前 6 条 -> 至少覆盖 25 条美妆。
