...
本次录屏如果实测 `gpt-5.5` 响应更快，可以只在启动命令里临时覆盖：
...
| 图片输入 | 上传安热沙/安耐晒防晒图片，再补充预算或油皮通勤需求 | 图片 -> image_plan -> 文本 RAG；不是图搜图 |
| 语音输入 | 录音说“帮我找 200 元内通勤防晒” | ASR 转写后进入输入框，用户确认再发送 |
| TTS / 无障碍 | 点击小助手播报，切换详细/不详细播报和字号设置 | TTS 播报、停止、低视力字号、状态可见 |
| 购买对象 | 底部对象 chip 从 `自己` 切到 `妈妈` / `爸爸` | 本轮购物对象上下文，不是切换助手人格 |

图片、语音、TTS 和购买对象建议只使用最终实测稳定的一组素材。若设备、provider 或 sidecar 不稳定，可在视频中展示设置/接口和边界，不要硬放失败素材。

## 6. Demo 视频对应操作路径

10 分钟以内完整录制版建议：

1. 展示真机 App 和 `/health`，说明是原生 Android + FastAPI，且本次录屏非 mock。
2. 用语音输入或手动输入防晒 query，展示 `quick_reply`、流式回答和商品卡片。
3. 展示商品卡片和详情页，强调价格/图片/详情来自数据源。
4. 上传安热沙/安耐晒防晒图，展示 image_plan 进入文本 RAG。
5. 切换购买对象并新建对话，展示上下文隔离和信息不足追问。
6. 展示多轮对比，说明上一轮商品 ID 如何支撑“第一款/第三款”。
7. 点击小助手播报，展示详细/不详细模式和无障碍设置。
8. 展示优惠/库存/下单 guardrail、trace 或 claim audit。
9. 展示架构图，解释 RAG、Prompt、guardrail、fallback、trace。
10. 说明边界：不做真实库存、优惠、下单、支付，不承诺零幻觉。
