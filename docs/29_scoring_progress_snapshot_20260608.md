...
| 效果与可靠性 | 20% | ◯ 有真实 API 证据，但仍是答辩重点 | 本轮真实 API `golden stream` 8/8 PASS；failure regression 11/11 PASS；conversation 7/7 PASS；Android 真实多轮复测通过防晒、T 恤、咖啡和信息不足追问 | 真实模型仍会触发 guardrail，答辩要讲清 repair / fallback 边界；groundedness real generation 仍不能说“全量稳定” |
| 加分项深度 | 20% | ◯ 主打方向清晰 | SKU 同系列规格、商品对比表、可解释 trace、轻量 graph-aware relation score、反馈闭环、claim-level audit 样例已有 | 不建议再开购物车、多模态、语音等大功能；优先把已有链路讲深 |
...
| failure regression | PASS | `scripts/run_failure_regression_cases.py`，11/11 PASS；新增 `FR-011` 锁住“多轮后泛护肤需求不能继承旧防晒/咖啡条件” |
...
| 意图识别 | ◯ | 规则 + planner 第一版；真实 API 验证通过核心路径，但复杂自然语言仍需保守 |
