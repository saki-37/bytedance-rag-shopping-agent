...
| 工程质量 | 25% | 代码结构、接口设计、错误处理、文档 | monorepo、README、API 契约、进度/安全/评测文档已有 | 结构清晰，仍需补架构图、Demo 脚本和关键运行说明 |
...
### P0-B：Constraint-Aware Hybrid Retrieval 收口
...
> Demo 稳定性收口：写 `docs/12_demo_script.md`、录第一版 1 分钟闭环。

原因：

1. 真实模型、商品卡片、详情、自动滚动和信息不足追问已经跑通，差一步就能形成可展示证据。
2. 快捷问题 chip 已解决中文输入不稳定，适合现场演示。
3. 完成录屏后，再扩数据或做 Graph-aware 会更踏实。
