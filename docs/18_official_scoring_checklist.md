...
| 提供流式 API，SSE 或 WebSocket 均可 | ✅ | `POST /api/chat/stream`，事件包括 status/quick_reply/products/token/done/error；推荐场景先用临时气泡接住需求，再发送完整 Planner + retrieval 的商品卡和正式回答 | 无明显缺口 |
